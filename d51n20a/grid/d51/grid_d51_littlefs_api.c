...
int littlefs_api_read(const struct lfs_config* c, lfs_block_t block, lfs_off_t off, void* buffer, lfs_size_t size) {
  ...
  if (err) {
    printf("failed to read addr %08x, size %08lx, err %d\n", offset, size, err);
    return err;
  }
...
int littlefs_api_prog(const struct lfs_config* c, lfs_block_t block, lfs_off_t off, const void* buffer, lfs_size_t size) {
  ...
  err = flash_append(dfs->flash, GRID_D51_NVM_BASE_ADDR + offset, (uint8_t*)buffer, size);
  if (err) {
    printf("failed to write addr %08x, size %08lx, err %d\n", offset, size, err);
    return err;
  }
