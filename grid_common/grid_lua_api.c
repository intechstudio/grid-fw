...
void grid_lua_set_memory_target(struct grid_lua_model *mod,
                                uint8_t target_kilobytes) {
...
void grid_lua_debug_memory_stats(struct grid_lua_model *mod, char *message) {
  ...
  uint32_t memusage = lua_gc(grid_lua_state.L, LUA_GCCOUNT) * 1024 +
                      lua_gc(grid_lua_state.L, LUA_GCCOUNTB);
...
/*static*/ int32_t grid_utility_map(int32_t x, int32_t in_min, int32_t in_max,
...
/*static*/ int l_grid_elementname_send(lua_State *L) {
  ...
  if (nargs == 2) {
    ...
    if (lua_type(L, 1) == LUA_TNUMBER && lua_type(L, 2) == LUA_TSTRING) {
      ...
      grid_msg_string_set_parameter(frame, GRID_INSTR_offset, GRID_INSTR_length,
                                    GRID_INSTR_EXECUTE_code, NULL);

      grid_msg_string_set_parameter(frame, GRID_CLASS_ELEMENTNAME_NUM_offset,
                                    GRID_CLASS_ELEMENTNAME_NUM_length, number,
                                    NULL);
      grid_msg_string_set_parameter(frame, GRID_CLASS_ELEMENTNAME_LENGTH_offset,
                                    GRID_CLASS_ELEMENTNAME_LENGTH_length,
                                    strlen(string), NULL);
...
/*static*/ int l_grid_usb_keyboard_send(lua_State *L) {
  ...
  grid_msg_string_set_parameter(temp, GRID_INSTR_offset, GRID_INSTR_length,
                                GRID_INSTR_EXECUTE_code, NULL);
  ...
  grid_msg_string_set_parameter(
      temp, GRID_CLASS_HIDKEYBOARD_DEFAULTDELAY_offset,
      GRID_CLASS_HIDKEYBOARD_DEFAULTDELAY_length, default_delay, NULL);
  ...
  for (int i = 2; i <= nargs; i += 3) {
    ...
    if (modifier == 15) {
      ...
      grid_msg_string_set_parameter(
          &temp[cursor], GRID_CLASS_HIDKEYBOARD_KEYISMODIFIER_offset,
          GRID_CLASS_HIDKEYBOARD_KEYISMODIFIER_length, modifier, NULL);
      grid_msg_string_set_parameter(
          &temp[cursor], GRID_CLASS_HIDKEYBOARD_DELAY_offset,
          GRID_CLASS_HIDKEYBOARD_DELAY_length, delay, NULL);
      cursor += 4;
      ...
      grid_msg_string_set_parameter(
          ...
          GRID_CLASS_HIDKEYBOARD_KEYISMODIFIER_length, modifier, NULL);
      grid_msg_string_set_parameter(
          &temp[cursor], GRID_CLASS_HIDKEYBOARD_KEYSTATE_offset,
          GRID_CLASS_HIDKEYBOARD_KEYSTATE_length, keystate, NULL);
      grid_msg_string_set_parameter(
          &temp[cursor], GRID_CLASS_HIDKEYBOARD_KEYCODE_offset,
          GRID_CLASS_HIDKEYBOARD_KEYCODE_length, keycode, NULL);
  ...
  grid_msg_string_set_parameter(temp, GRID_CLASS_HIDKEYBOARD_LENGTH_offset,
                                GRID_CLASS_HIDKEYBOARD_LENGTH_length,
                                cursor / 4 + 1, NULL);
...
/*static*/ int l_grid_mousemove_send(lua_State *L) {
  ...
  grid_msg_string_set_parameter(frame, GRID_CLASS_HIDMOUSEMOVE_POSITION_offset,
                                GRID_CLASS_HIDMOUSEMOVE_POSITION_length,
                                position, NULL);
  grid_msg_string_set_parameter(frame, GRID_CLASS_HIDMOUSEMOVE_AXIS_offset,
                                GRID_CLASS_HIDMOUSEMOVE_AXIS_length, axis,
                                NULL);
...
/*static*/ int l_grid_mousebutton_send(lua_State *L) {
  ...
  grid_msg_string_set_parameter(frame, GRID_CLASS_HIDMOUSEBUTTON_STATE_offset,
                                GRID_CLASS_HIDMOUSEBUTTON_STATE_length, state,
                                NULL);
  grid_msg_string_set_parameter(frame, GRID_CLASS_HIDMOUSEBUTTON_BUTTON_offset,
                                GRID_CLASS_HIDMOUSEBUTTON_BUTTON_length, button,
                                NULL);
...
/*static*/ int l_grid_gamepadmove_send(lua_State *L) {
  ...
  grid_msg_string_set_parameter(frame, GRID_CLASS_HIDGAMEPADMOVE_AXIS_offset,
                                GRID_CLASS_HIDGAMEPADMOVE_AXIS_length, axis,
                                NULL);
  grid_msg_string_set_parameter(
      frame, GRID_CLASS_HIDGAMEPADMOVE_POSITION_offset,
      GRID_CLASS_HIDGAMEPADMOVE_POSITION_length, position, NULL);
...
/*static*/ int l_grid_gamepadbutton_send(lua_State *L) {
  ...
  grid_msg_string_set_parameter(
      frame, GRID_CLASS_HIDGAMEPADBUTTON_BUTTON_offset,
      GRID_CLASS_HIDGAMEPADBUTTON_BUTTON_length, button, NULL);
  grid_msg_string_set_parameter(frame, GRID_CLASS_HIDGAMEPADBUTTON_STATE_offset,
                                GRID_CLASS_HIDGAMEPADBUTTON_STATE_length, state,
                                NULL);
...
/*static*/ int l_grid_midi_send(lua_State *L) {
  ...
  grid_msg_string_set_parameter(midiframe, GRID_INSTR_offset, GRID_INSTR_length,
                                GRID_INSTR_EXECUTE_code, NULL);

  grid_msg_string_set_parameter(midiframe, GRID_CLASS_MIDI_CHANNEL_offset,
                                GRID_CLASS_MIDI_CHANNEL_length, channel, NULL);
  grid_msg_string_set_parameter(midiframe, GRID_CLASS_MIDI_COMMAND_offset,
                                GRID_CLASS_MIDI_COMMAND_length, command, NULL);
  grid_msg_string_set_parameter(midiframe, GRID_CLASS_MIDI_PARAM1_offset,
                                GRID_CLASS_MIDI_PARAM1_length, param1, NULL);
  grid_msg_string_set_parameter(midiframe, GRID_CLASS_MIDI_PARAM2_offset,
                                GRID_CLASS_MIDI_PARAM2_length, param2, NULL);
...
/*static*/ int l_grid_midi_sysex_send(lua_State *L) {
  ...
  for (i = 1; i <= nargs; ++i) {
    ...
    grid_msg_string_set_parameter(
        midiframe, GRID_CLASS_MIDISYSEX_PAYLOAD_offset + i * 2 - 2, 2,
        lua_tointeger(L, i), NULL);
  }

  grid_msg_string_set_parameter(midiframe, GRID_CLASS_MIDISYSEX_LENGTH_offset,
                                GRID_CLASS_MIDISYSEX_LENGTH_length, i - 1,
                                NULL);
...
/*static*/ int l_grid_led_layer_min(lua_State *L) {
  ...
  grid_led_set_layer_min(&grid_led_state, param[0], param[1], param[2],
                         param[3], param[4]);
...
/*static*/ int l_grid_led_layer_mid(lua_State *L) {
  ...
  grid_led_set_layer_mid(&grid_led_state, param[0], param[1], param[2],
                         param[3], param[4]);
...
/*static*/ int l_grid_led_layer_max(lua_State *L) {
  ...
  grid_led_set_layer_max(&grid_led_state, param[0], param[1], param[2],
                         param[3], param[4]);
...
/*static*/ int l_grid_led_layer_timeout(lua_State *L) {
  ...
  grid_led_set_layer_timeout(&grid_led_state, (uint8_t)param[0],
                             (uint8_t)param[1], param[2]);
  return 0;
}
  ...
  if (nargs == 3) {
    ...
    if (param[2] > -1) { // phase is a phase value
      ...
      struct grid_ui_element *ele =
          grid_ui_element_find(&grid_ui_state, param[0]);
      ...
      if (ele_type == GRID_UI_ELEMENT_POTENTIOMETER) {
        ...
        min = grid_ui_element_get_template_parameter(
            ele, GRID_LUA_FNC_P_POTMETER_MIN_index);
        max = grid_ui_element_get_template_parameter(
            ele, GRID_LUA_FNC_P_POTMETER_MAX_index);
        val = grid_ui_element_get_template_parameter(
            ele, GRID_LUA_FNC_P_POTMETER_VALUE_index);
      } else if (ele_type == GRID_UI_ELEMENT_ENCODER) {

        min = grid_ui_element_get_template_parameter(
            ele, GRID_LUA_FNC_E_ENCODER_MIN_index);
        max = grid_ui_element_get_template_parameter(
            ele, GRID_LUA_FNC_E_ENCODER_MAX_index);
        val = grid_ui_element_get_template_parameter(
            ele, GRID_LUA_FNC_E_ENCODER_VALUE_index);
      } else {
...
/*static*/ int l_grid_page_load(lua_State *L) {
  ...
  if (grid_ui_page_change_is_enabled(&grid_ui_state)) {
    ...
    if (grid_ui_bulk_pageread_is_in_progress(&grid_ui_state) == 0) {
      ...
      grid_msg_string_set_parameter(response, GRID_INSTR_offset,
                                    GRID_INSTR_length, GRID_INSTR_EXECUTE_code,
                                    NULL);
      grid_msg_string_set_parameter(
          response, GRID_CLASS_PAGEACTIVE_PAGENUMBER_offset,
          GRID_CLASS_PAGEACTIVE_PAGENUMBER_length, page, NULL);
...
/*static*/ int l_grid_event_trigger(lua_State *L) {
  ...
  if (ele != NULL) {
    ...
    if (eve != NULL) {
      ...
      sprintf(&grid_lua_state.stde[strlen(grid_lua_state.stde) - 1], "%ld %ld",
              param[0], param[1]);
    }
...
/*static*/ const struct luaL_Reg printlib[] = {
    ...
    {GRID_LUA_FNC_G_MOUSEBUTTON_SEND_short,
     GRID_LUA_FNC_G_MOUSEBUTTON_SEND_fnptr},

    {GRID_LUA_FNC_G_GAMEPADMOVE_SEND_short,
     GRID_LUA_FNC_G_GAMEPADMOVE_SEND_fnptr},
    {GRID_LUA_FNC_G_GAMEPADBUTTON_SEND_short,
     GRID_LUA_FNC_G_GAMEPADBUTTON_SEND_fnptr},
    ...
    {GRID_LUA_FNC_G_ELEMENTNAME_SEND_short,
     GRID_LUA_FNC_G_ELEMENTNAME_SEND_fnptr},
...
void grid_lua_ui_init_po16(struct grid_lua_model *mod) {
  ...
  grid_lua_dostring(mod, "for i=0, 15 do " GRID_LUA_KW_ELEMENT_short
                         "[i] = {index = i} end");
  grid_lua_dostring(mod,
                    "for i=0, 15 do setmetatable(" GRID_LUA_KW_ELEMENT_short
                    "[i], potmeter_meta) end");
  ...
  grid_lua_dostring(mod, "setmetatable(" GRID_LUA_KW_ELEMENT_short
                         "[16], system_meta)");
}
...
void grid_lua_ui_init_pbf4(struct grid_lua_model *mod) {
  ...
  grid_lua_dostring(mod, "for i=0, 7  do " GRID_LUA_KW_ELEMENT_short
                         "[i] = {index = i} end");
  grid_lua_dostring(mod,
                    "for i=0, 7  do  setmetatable(" GRID_LUA_KW_ELEMENT_short
                    "[i], potmeter_meta)  end");

  grid_lua_dostring(mod, "for i=8, 11 do " GRID_LUA_KW_ELEMENT_short
                         "[i] = {index = i} end");
  grid_lua_dostring(mod,
                    "for i=8, 11 do  setmetatable(" GRID_LUA_KW_ELEMENT_short
                    "[i], button_meta)  end");
...
void grid_lua_ui_init_pb44(struct grid_lua_model *mod) {
  ...
  grid_lua_dostring(mod, "for i=8, 15 do " GRID_LUA_KW_ELEMENT_short
                         "[i] = {index = i} end");
  grid_lua_dostring(mod,
                    "for i=8, 15 do  setmetatable(" GRID_LUA_KW_ELEMENT_short
                    "[i], button_meta)  end");
...
void grid_lua_ui_init_ef44(struct grid_lua_model *mod) {
  ...
  grid_lua_dostring(mod, "for i=0, 3  do " GRID_LUA_KW_ELEMENT_short
                         "[i] = {index = i} end");
  grid_lua_dostring(mod,
                    "for i=0, 3  do  setmetatable(" GRID_LUA_KW_ELEMENT_short
                    "[i], encoder_meta)  end");

  grid_lua_dostring(mod, "for i=4, 7 do " GRID_LUA_KW_ELEMENT_short
                         "[i] = {index = i} end");
  grid_lua_dostring(mod,
                    "for i=4, 7 do  setmetatable(" GRID_LUA_KW_ELEMENT_short
                    "[i], potmeter_meta)  end");
...
void grid_lua_ui_init_tek2(struct grid_lua_model *mod) {
  ...
  grid_lua_dostring(mod, "for i=0, 7 do " GRID_LUA_KW_ELEMENT_short
                         "[i] = {index = i} end");
  grid_lua_dostring(mod, "for i=0, 7 do setmetatable(" GRID_LUA_KW_ELEMENT_short
                         "[i], button_meta) end");

  // initialize 2 endless potentiometers as encoders
  grid_lua_dostring(mod, "for i=8, 9  do " GRID_LUA_KW_ELEMENT_short
                         "[i] = {index = i} end");
  grid_lua_dostring(mod,
                    "for i=8, 9  do  setmetatable(" GRID_LUA_KW_ELEMENT_short
                    "[i], encoder_meta)  end");
...
void grid_lua_ui_init(struct grid_lua_model *lua, struct grid_ui_model *ui) {
  ...
  if (ui->lua_ui_init_callback == NULL) {
    ...
    grid_platform_printf(
        "LUA UI INIT FAILED: ui->lua_ui_init_callback not registered\r\n");
    return;
  }
