...
void grid_usb_hid_init(struct grid_usb_hid_model* usb_hid) {
  grid_usb_keyboard_init(&usb_hid->keyboard);
  grid_usb_mouse_init(&usb_hid->mouse);
  grid_usb_macro_init(&usb_hid->macro, GRID_MACRO_TX_BUFFER_SIZE, &usb_hid->keyboard, &usb_hid->mouse);
  grid_usb_gamepad_init(&usb_hid->gamepad, GRID_GAMEPAD_TX_BUFFER_SIZE);
}
