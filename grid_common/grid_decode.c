...
static uint8_t grid_check_destination(char *header,
                                      uint8_t target_destination_bm) {
  ...
  uint8_t dx = grid_msg_string_get_parameter(header, GRID_BRC_DX_offset,
                                             GRID_BRC_DX_length, &error);
  uint8_t dy = grid_msg_string_get_parameter(header, GRID_BRC_DY_offset,
                                             GRID_BRC_DY_length, &error);

  uint8_t header_destination_bm = 0;

  if (dx == GRID_PARAMETER_DEFAULT_POSITION &&
      dy == GRID_PARAMETER_DEFAULT_POSITION) {
    // position_is_me = 1;
    header_destination_bm |= GRID_DESTINATION_IS_ME;
  } else if (dx == GRID_PARAMETER_GLOBAL_POSITION &&
             dy == GRID_PARAMETER_GLOBAL_POSITION) {
    // position_is_global = 1;
    header_destination_bm |= GRID_DESTINATION_IS_GLOBAL;
  } else if (dx == GRID_PARAMETER_LOCAL_POSITION &&
             dy == GRID_PARAMETER_LOCAL_POSITION) {
    // position_is_local = 1;
...
uint8_t grid_decode_midi_to_usb(char *header, char *chunk) {
  ...
  uint8_t msg_instr = grid_msg_string_get_parameter(chunk, GRID_INSTR_offset,
                                                    GRID_INSTR_length, &error);
  if (msg_instr != GRID_INSTR_EXECUTE_code) {
    return 1;
  }

  uint8_t midi_channel =
      grid_msg_string_get_parameter(chunk, GRID_CLASS_MIDI_CHANNEL_offset,
                                    GRID_CLASS_MIDI_CHANNEL_length, &error);
  uint8_t midi_command =
      grid_msg_string_get_parameter(chunk, GRID_CLASS_MIDI_COMMAND_offset,
                                    GRID_CLASS_MIDI_COMMAND_length, &error);
  uint8_t midi_param1 =
      grid_msg_string_get_parameter(chunk, GRID_CLASS_MIDI_PARAM1_offset,
                                    GRID_CLASS_MIDI_PARAM1_length, &error);
  uint8_t midi_param2 =
      grid_msg_string_get_parameter(chunk, GRID_CLASS_MIDI_PARAM2_offset,
                                    GRID_CLASS_MIDI_PARAM2_length, &error);
...
uint8_t grid_decode_sysex_to_usb(char *header, char *chunk) {
  ...
  uint16_t length =
      grid_msg_string_get_parameter(chunk, GRID_CLASS_MIDISYSEX_LENGTH_offset,
                                    GRID_CLASS_MIDISYSEX_LENGTH_length, &error);
  ...
  uint8_t first = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_MIDISYSEX_PAYLOAD_offset,
      GRID_CLASS_MIDISYSEX_PAYLOAD_length, &error);
  uint8_t last = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_MIDISYSEX_PAYLOAD_offset + (length - 1) * 2,
      GRID_CLASS_MIDISYSEX_PAYLOAD_length, &error);
  ...
  for (uint16_t i = 0; i < length;) {
    ...
    midievent.byte1 = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_MIDISYSEX_PAYLOAD_offset + i * 2,
        GRID_CLASS_MIDISYSEX_PAYLOAD_length, &error);
    i++;
    if (i < length) {
      midievent.byte2 = grid_msg_string_get_parameter(
          chunk, GRID_CLASS_MIDISYSEX_PAYLOAD_offset + i * 2,
          GRID_CLASS_MIDISYSEX_PAYLOAD_length, &error);
      i++;
    }
    if (i < length) {
      midievent.byte3 = grid_msg_string_get_parameter(
          chunk, GRID_CLASS_MIDISYSEX_PAYLOAD_offset + i * 2,
          GRID_CLASS_MIDISYSEX_PAYLOAD_length, &error);
      i++;
    }
    ...
    // grid_port_debug_printf("Packet: %d %d %d %d", midievent.byte0,
    // midievent.byte1, midievent.byte2, midievent.byte3);
  ...
  if (number_of_packets_dropped) {
    grid_port_debug_printf("MIDI TX: %d Packet(s) Dropped!",
...
uint8_t grid_decode_mousebutton_to_usb(char *header, char *chunk) {
  ...
  uint8_t state = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDMOUSEBUTTON_STATE_offset,
      GRID_CLASS_HIDMOUSEBUTTON_STATE_length, &error);
  uint8_t button = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDMOUSEBUTTON_BUTTON_offset,
      GRID_CLASS_HIDMOUSEBUTTON_BUTTON_length, &error);
...
uint8_t grid_decode_mousemove_to_usb(char *header, char *chunk) {
  ...
  uint8_t position_raw = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDMOUSEMOVE_POSITION_offset,
      GRID_CLASS_HIDMOUSEMOVE_POSITION_length, &error);
  uint8_t axis = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDMOUSEMOVE_AXIS_offset,
      GRID_CLASS_HIDMOUSEMOVE_AXIS_length, &error);
...
uint8_t grid_decode_gamepadmove_to_usb(char *header, char *chunk) {
  ...
  uint8_t axis = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDGAMEPADMOVE_AXIS_offset,
      GRID_CLASS_HIDGAMEPADMOVE_AXIS_length, &error);
  uint8_t position_raw = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDGAMEPADMOVE_POSITION_offset,
      GRID_CLASS_HIDGAMEPADMOVE_POSITION_length, &error);
...
uint8_t grid_decode_gamepadbutton_to_usb(char *header, char *chunk) {
  ...
  uint8_t button = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDGAMEPADBUTTON_BUTTON_offset,
      GRID_CLASS_HIDGAMEPADBUTTON_BUTTON_length, &error);
  uint8_t state = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDGAMEPADBUTTON_STATE_offset,
      GRID_CLASS_HIDGAMEPADBUTTON_STATE_length, &error);
...
uint8_t grid_decode_keyboard_to_usb(char *header, char *chunk) {
  ...
  uint16_t length = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDKEYBOARD_LENGTH_offset,
      GRID_CLASS_HIDKEYBOARD_LENGTH_length, &error);

  uint8_t default_delay = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDKEYBOARD_DEFAULTDELAY_offset,
      GRID_CLASS_HIDKEYBOARD_DEFAULTDELAY_length, &error);

  uint32_t number_of_packets_dropped = 0;

  for (uint16_t j = 0; j < length * 4; j += 4) {

    uint8_t key_ismodifier = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_HIDKEYBOARD_KEYISMODIFIER_offset + j,
        GRID_CLASS_HIDKEYBOARD_KEYISMODIFIER_length, &error);
    uint8_t key_state = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_HIDKEYBOARD_KEYSTATE_offset + j,
        GRID_CLASS_HIDKEYBOARD_KEYSTATE_length, &error);
    uint8_t key_code = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_HIDKEYBOARD_KEYCODE_offset + j,
        GRID_CLASS_HIDKEYBOARD_KEYCODE_length, &error);
    ...
    if (key_ismodifier == 0 || key_ismodifier == 1) {
      ...
      if (key_state == 2) { // combined press and release
        ...
        key.ispressed = 1;
        number_of_packets_dropped +=
            grid_usb_keyboard_tx_push(&grid_usb_keyboard_state, key);
        key.ispressed = 0;
        number_of_packets_dropped +=
            grid_usb_keyboard_tx_push(&grid_usb_keyboard_state, key);
      ...
      uint16_t delay = grid_msg_string_get_parameter(
          chunk, GRID_CLASS_HIDKEYBOARD_DELAY_offset + j,
          GRID_CLASS_HIDKEYBOARD_DELAY_length, &error);
  ...
  if (number_of_packets_dropped) {
    grid_port_debug_printf("KEYBOARD: %d Packet(s) Dropped!",
                           number_of_packets_dropped);
  };
...
uint8_t grid_decode_pageactive_to_ui(char *header, char *chunk) {
  ...
  uint8_t msg_instr = grid_msg_string_get_parameter(chunk, GRID_INSTR_offset,
                                                    GRID_INSTR_length, &error);
  uint8_t page = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_PAGEACTIVE_PAGENUMBER_offset,
      GRID_CLASS_PAGEACTIVE_PAGENUMBER_length, &error);
  ...
  if (msg_instr == GRID_INSTR_EXECUTE_code) { // SET BANK
    ...
    uint8_t sx = grid_msg_string_get_parameter(header, GRID_BRC_SX_offset,
                                               GRID_BRC_SX_length, &error);
    uint8_t sy = grid_msg_string_get_parameter(header, GRID_BRC_SY_offset,
                                               GRID_BRC_SY_length, &error);

    if (sx == GRID_PARAMETER_DEFAULT_POSITION &&
        sy == GRID_PARAMETER_DEFAULT_POSITION) {
...
uint8_t grid_decode_pagecount_to_ui(char *header, char *chunk) {
  ...
  if (grid_check_destination(header, GRID_DESTINATION_IS_ME |
                                         GRID_DESTINATION_IS_GLOBAL) == false) {
    return 1;
  }
  ...
  if (msg_instr == GRID_INSTR_FETCH_code) {
    ...
    grid_msg_packet_body_set_parameter(&response, 0, GRID_INSTR_offset,
                                       GRID_INSTR_length,
                                       GRID_INSTR_REPORT_code);
    grid_msg_packet_body_set_parameter(
        &response, 0, GRID_CLASS_PAGECOUNT_PAGENUMBER_offset,
        GRID_CLASS_PAGECOUNT_PAGENUMBER_length, grid_ui_state.page_count);
...
uint8_t grid_decode_midi_to_ui(char *header, char *chunk) {
  ...
  if (msg_instr == GRID_INSTR_REPORT_code) {
    ...
    struct grid_ui_element *ele =
        &grid_ui_state.element_list[grid_ui_state.element_list_length - 1];
...
uint8_t grid_decode_imediate_to_ui(char *header, char *chunk) {
  ...
  if (grid_check_destination(header, GRID_DESTINATION_IS_ME |
                                         GRID_DESTINATION_IS_GLOBAL |
                                         GRID_DESTINATION_IS_LOCAL) == false) {
    return 1;
  }
  ...
  if (msg_instr == GRID_INSTR_EXECUTE_code) {
    ...
    uint16_t length = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_IMEDIATE_ACTIONLENGTH_offset,
        GRID_CLASS_IMEDIATE_ACTIONLENGTH_length, &error);
...
uint8_t grid_decode_heartbeat_to_ui(char *header, char *chunk) {
  ...
  uint8_t type =
      grid_msg_string_get_parameter(chunk, GRID_CLASS_HEARTBEAT_TYPE_offset,
                                    GRID_CLASS_HEARTBEAT_TYPE_length, &error);
  ...
  if (type == 0) {
    ...
    uint8_t sy = grid_msg_string_get_parameter(header, GRID_BRC_SY_offset,
                                               GRID_BRC_SY_length, &error);
    uint8_t rot = grid_msg_string_get_parameter(header, GRID_BRC_ROT_offset,
                                                GRID_BRC_ROT_length, &error);
    uint8_t portrot = grid_msg_string_get_parameter(
        header, GRID_BRC_PORTROT_offset, GRID_BRC_PORTROT_length, &error);

    grid_sys_set_module_absolute_position(&grid_sys_state, sx, sy, rot,
                                          portrot);
    ...
    grid_msg_set_editor_heartbeat_lastrealtime(&grid_msg_state,
                                               grid_platform_rtc_get_micros());
    ...
    if (editor_connected_now) {
      ...
      uint16_t len = 0;
      snprintf(&response_payload[len], 299,
               GRID_CLASS_EVENTPREVIEW_frame_start);
      ...
      for (uint8_t j = 0; j < grid_ui_state.element_list_length; j++) {
        ...
        if (ele->type == GRID_UI_ELEMENT_POTENTIOMETER) {
          ...
          element_value =
              ele->template_parameter_list[GRID_LUA_FNC_P_POTMETER_VALUE_index];
        } else if (ele->type == GRID_UI_ELEMENT_ENCODER) {

          element_value =
              ele->template_parameter_list[GRID_LUA_FNC_E_ENCODER_VALUE_index];
        }
      ...
      grid_msg_packet_body_set_parameter(&response, 0, GRID_INSTR_offset,
                                         ...
                                         GRID_INSTR_REPORT_code);
      grid_msg_packet_body_set_parameter(
          &response, 0, GRID_CLASS_EVENTPREVIEW_LENGTH_offset,
          GRID_CLASS_EVENTPREVIEW_LENGTH_length, report_length);
    ...
    if (grid_protocol_led_change_report_length(&grid_led_state)) {
      ...
      uint16_t report_length = grid_protocol_led_change_report_generate(
          &grid_led_state, -1, &response_payload[len]);
      ...
      grid_msg_packet_body_append_printf(&response,
                                         GRID_CLASS_LEDPREVIEW_frame_end);

      grid_msg_packet_body_set_parameter(&response, 0, GRID_INSTR_offset,
                                         GRID_INSTR_length,
                                         GRID_INSTR_REPORT_code);
      grid_msg_packet_body_set_parameter(
          &response, 0, GRID_CLASS_LEDPREVIEW_LENGTH_offset,
          GRID_CLASS_LEDPREVIEW_LENGTH_length, report_length);
...
uint8_t grid_decode_serialmuber_to_ui(char *header, char *chunk) {
  ...
  if (msg_instr == GRID_INSTR_FETCH_code) {
    ...
    grid_msg_packet_body_set_parameter(
        &response, 0, GRID_CLASS_SERIALNUMBER_WORD0_offset,
        GRID_CLASS_SERIALNUMBER_WORD0_length, uniqueid[0]);
    grid_msg_packet_body_set_parameter(
        &response, 0, GRID_CLASS_SERIALNUMBER_WORD1_offset,
        GRID_CLASS_SERIALNUMBER_WORD1_length, uniqueid[1]);
    grid_msg_packet_body_set_parameter(
        &response, 0, GRID_CLASS_SERIALNUMBER_WORD2_offset,
        GRID_CLASS_SERIALNUMBER_WORD2_length, uniqueid[2]);
    grid_msg_packet_body_set_parameter(
        &response, 0, GRID_CLASS_SERIALNUMBER_WORD3_offset,
        GRID_CLASS_SERIALNUMBER_WORD3_length, uniqueid[3]);
...
uint8_t grid_decode_uptime_to_ui(char *header, char *chunk) {
  ...
  if (msg_instr == GRID_INSTR_FETCH_code) {
    ...
    grid_msg_packet_body_set_parameter(
        &response, 0, GRID_CLASS_UPTIME_UPTIME_offset,
        GRID_CLASS_UPTIME_UPTIME_length, (uint32_t)uptime);
...
uint8_t grid_decode_resetcause_to_ui(char *header, char *chunk) {
  ...
  if (msg_instr == GRID_INSTR_FETCH_code) {
    ...
    grid_msg_packet_body_set_parameter(
        &response, 0, GRID_CLASS_RESETCAUSE_CAUSE_offset,
        GRID_CLASS_RESETCAUSE_CAUSE_length, grid_platform_get_reset_cause());
...
uint8_t grid_decode_pagediscard_to_ui(char *header, char *chunk) {
  ...
  if (msg_instr == GRID_INSTR_EXECUTE_code) {
    ...
    uint8_t id = grid_msg_string_get_parameter(header, GRID_BRC_ID_offset,
                                               GRID_BRC_ID_length, &error);

    grid_msg_store_lastheader(&grid_msg_state,
                              GRID_MSG_LASTHEADER_DISCARD_INDEX, id);

    if (grid_ui_bulk_pageread_is_in_progress(&grid_ui_state)) {

    } else {
      grid_ui_bulk_pageread_init(&grid_ui_state,
                                 &grid_protocol_nvm_read_succcess_callback);
    }

  } else if (msg_instr == GRID_INSTR_CHECK_code) {

    uint8_t state = grid_msg_get_lastheader_state(
        &grid_msg_state, GRID_MSG_LASTHEADER_DISCARD_INDEX);
    uint8_t id = grid_msg_get_lastheader_id(&grid_msg_state,
    ...
    grid_msg_packet_body_append_parameter(
        &response, GRID_CLASS_PAGEDISCARD_LASTHEADER_offset,
        GRID_CLASS_PAGEDISCARD_LASTHEADER_length, id);

    uint8_t acknowledge = (state == 0) ? GRID_INSTR_ACKNOWLEDGE_code
                                       : GRID_INSTR_NACKNOWLEDGE_code;
    grid_msg_packet_body_append_parameter(&response, GRID_INSTR_offset,
                                          GRID_INSTR_length, acknowledge);
...
uint8_t grid_decode_pagestore_to_ui(char *header, char *chunk) {
  ...
  uint8_t dx = grid_msg_string_get_parameter(header, GRID_BRC_DX_offset,
                                             GRID_BRC_DX_length, NULL);
  uint8_t dy = grid_msg_string_get_parameter(header, GRID_BRC_DY_offset,
                                             GRID_BRC_DY_length, NULL);
  ...
  if (msg_instr == GRID_INSTR_EXECUTE_code) {
    ...
    grid_msg_store_lastheader(&grid_msg_state, GRID_MSG_LASTHEADER_STORE_INDEX,
                              id);
    ...
    grid_ui_bulk_pagestore_init(&grid_ui_state,
                                &grid_protocol_nvm_store_succcess_callback);

  } else if (msg_instr == GRID_INSTR_CHECK_code) {

    uint8_t state = grid_msg_get_lastheader_state(
        &grid_msg_state, GRID_MSG_LASTHEADER_STORE_INDEX);
    uint8_t id = grid_msg_get_lastheader_id(&grid_msg_state,
    ...
    grid_msg_packet_body_append_parameter(
        &response, GRID_CLASS_PAGESTORE_LASTHEADER_offset,
        GRID_CLASS_PAGESTORE_LASTHEADER_length, id);
...
uint8_t grid_decode_pageclear_to_ui(char *header, char *chunk) {
  ...
  if (msg_instr == GRID_INSTR_EXECUTE_code) {
    ...
    uint8_t state = grid_msg_get_lastheader_state(
        &grid_msg_state, GRID_MSG_LASTHEADER_CLEAR_INDEX);
    uint8_t id = grid_msg_get_lastheader_id(&grid_msg_state,
    ...
    grid_msg_packet_body_append_parameter(
        &response, GRID_CLASS_PAGECLEAR_LASTHEADER_offset,
        GRID_CLASS_PAGECLEAR_LASTHEADER_length, id);
...
uint8_t grid_decode_nvmerase_to_ui(char *header, char *chunk) {
  ...
  if (msg_instr == GRID_INSTR_EXECUTE_code) {
    ...
    grid_msg_store_lastheader(&grid_msg_state, GRID_MSG_LASTHEADER_ERASE_INDEX,
                              id);
    grid_ui_bulk_nvmerase_init(&grid_ui_state,
                               &grid_protocol_nvm_erase_succcess_callback);
    ...
    for (uint8_t i = 0; i < grid_led_get_led_count(&grid_led_state); i++) {
      grid_led_set_layer_min(&grid_led_state, i, GRID_LED_LAYER_ALERT,
                             GRID_LED_COLOR_YELLOW_DIM);
    }

  } else if (msg_instr == GRID_INSTR_CHECK_code) {

    uint8_t state = grid_msg_get_lastheader_state(
        &grid_msg_state, GRID_MSG_LASTHEADER_ERASE_INDEX);
    uint8_t id = grid_msg_get_lastheader_id(&grid_msg_state,
    ...
    grid_msg_packet_body_append_parameter(
        &response, GRID_CLASS_NVMERASE_LASTHEADER_offset,
        GRID_CLASS_NVMERASE_LASTHEADER_length, id);
...
uint8_t grid_decode_config_to_ui(char *header, char *chunk) {
  ...
  if (msg_instr == GRID_INSTR_EXECUTE_code) {
    ...
    grid_msg_store_lastheader(&grid_msg_state, GRID_MSG_LASTHEADER_CONFIG_INDEX,
                              id);
    ...
    uint8_t vmajor = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_CONFIG_VERSIONMAJOR_offset,
        GRID_CLASS_CONFIG_VERSIONMAJOR_length, NULL);
    uint8_t vminor = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_CONFIG_VERSIONMINOR_offset,
        GRID_CLASS_CONFIG_VERSIONMINOR_length, NULL);
    uint8_t vpatch = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_CONFIG_VERSIONPATCH_offset,
        GRID_CLASS_CONFIG_VERSIONPATCH_length, NULL);

    uint8_t pagenumber = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_CONFIG_PAGENUMBER_offset,
        GRID_CLASS_CONFIG_PAGENUMBER_length, NULL);
    uint8_t elementnumber = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_CONFIG_ELEMENTNUMBER_offset,
        GRID_CLASS_CONFIG_ELEMENTNUMBER_length, NULL);
    uint8_t eventtype =
        grid_msg_string_get_parameter(chunk, GRID_CLASS_CONFIG_EVENTTYPE_offset,
                                      GRID_CLASS_CONFIG_EVENTTYPE_length, NULL);
    uint16_t actionlength = grid_msg_string_get_parameter(
        chunk, GRID_CLASS_CONFIG_ACTIONLENGTH_offset,
    ...
    if (action[actionlength] == GRID_CONST_ETX) {
      ...
      if (actionlength < GRID_PARAMETER_ACTIONSTRING_maxlength) {
        ...
        // printf("Config: %d %d %d %d -> %s\r\n", pagenumber, elementnumber,
        // eventtype, actionlength, action);

        grid_alert_all_set(&grid_led_state, GRID_LED_COLOR_WHITE, 64);
        if (pagenumber == grid_ui_state.page_activepage) {

          // find event
          struct grid_ui_event *eve = grid_ui_event_find(
              &grid_ui_state.element_list[elementnumber], eventtype);
      ...
      printf("Config frame invalid: %d %d %d %d end: %d %s\r\n", pagenumber,
             elementnumber, eventtype, actionlength, action[actionlength],
             chunk);
    }
    ...
    grid_msg_packet_body_append_printf(&response,
                                       GRID_CLASS_CONFIG_frame_check);

    grid_msg_packet_body_append_parameter(
        &response, GRID_CLASS_CONFIG_LASTHEADER_offset,
        GRID_CLASS_CONFIG_LASTHEADER_length, id);

    grid_msg_clear_lastheader(&grid_msg_state,
                              GRID_MSG_LASTHEADER_CONFIG_INDEX);

    grid_msg_packet_body_append_parameter(&response, GRID_INSTR_offset,
                                          GRID_INSTR_length, ack);
    ...
    grid_ui_event_recall_configuration(&grid_ui_state, pagenumber,
                                       elementnumber, eventtype, temp);
    ...
    if (strlen(temp) != 0) {
      ...
      grid_msg_packet_init(&grid_msg_state, &message,
                           GRID_PARAMETER_GLOBAL_POSITION,
                           GRID_PARAMETER_GLOBAL_POSITION);

      grid_msg_packet_body_append_printf(&message,
                                         GRID_CLASS_CONFIG_frame_start);

      grid_msg_packet_body_append_parameter(&message, GRID_INSTR_offset,
                                            GRID_INSTR_length,
                                            GRID_INSTR_REPORT_code);

      grid_msg_packet_body_append_parameter(
          &message, GRID_CLASS_CONFIG_VERSIONMAJOR_offset,
          GRID_CLASS_CONFIG_VERSIONMAJOR_length, GRID_PROTOCOL_VERSION_MAJOR);
      grid_msg_packet_body_append_parameter(
          &message, GRID_CLASS_CONFIG_VERSIONMINOR_offset,
          GRID_CLASS_CONFIG_VERSIONMINOR_length, GRID_PROTOCOL_VERSION_MINOR);
      grid_msg_packet_body_append_parameter(
          &message, GRID_CLASS_CONFIG_VERSIONPATCH_offset,
          GRID_CLASS_CONFIG_VERSIONPATCH_length, GRID_PROTOCOL_VERSION_PATCH);
      ...
      grid_msg_packet_body_append_parameter(
          &message, GRID_CLASS_CONFIG_PAGENUMBER_offset,
          GRID_CLASS_CONFIG_PAGENUMBER_length, pagenumber);
      grid_msg_packet_body_append_parameter(
          &message, GRID_CLASS_CONFIG_ELEMENTNUMBER_offset,
          GRID_CLASS_CONFIG_EVENTTYPE_length, element_helper);
      grid_msg_packet_body_append_parameter(
          &message, GRID_CLASS_CONFIG_EVENTTYPE_offset,
          GRID_CLASS_CONFIG_EVENTTYPE_length, eventtype);
      grid_msg_packet_body_append_parameter(
          &message, GRID_CLASS_CONFIG_ACTIONLENGTH_offset,
          GRID_CLASS_CONFIG_ACTIONLENGTH_length, 0);

      grid_msg_packet_body_append_parameter(
          &message, GRID_CLASS_CONFIG_ACTIONLENGTH_offset,
          GRID_CLASS_CONFIG_ACTIONLENGTH_length, strlen(temp));
...
uint8_t grid_decode_hidkeystatus_to_ui(char *header, char *chunk) {
  ...
  uint8_t isenabled = grid_msg_string_get_parameter(
      chunk, GRID_CLASS_HIDKEYSTATUS_ISENABLED_offset,
      GRID_CLASS_HIDKEYSTATUS_ISENABLED_length, &error);
  ...
  if (msg_instr == GRID_INSTR_EXECUTE_code) {
    ...
    grid_msg_packet_body_append_parameter(
        &response, GRID_CLASS_HIDKEYSTATUS_ISENABLED_offset,
        GRID_CLASS_HIDKEYSTATUS_ISENABLED_length,
        grid_usb_keyboard_isenabled(&grid_usb_keyboard_state));
