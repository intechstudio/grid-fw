...
static void IRAM_ATTR my_post_trans_cb(spi_transaction_t *trans) {
  ...
  xRingbufferSendFromISR(grid_esp32_encoder_state.ringbuffer_handle, &result,
                         sizeof(struct grid_esp32_encoder_result), NULL);
...
void grid_esp32_module_ef44_task(void *arg) {
  grid_esp32_encoder_init(&grid_esp32_encoder_state, my_post_setup_cb,
                          my_post_trans_cb);
  ...
  while (1) {
    ...
    adc_result = (struct grid_esp32_adc_result *)xRingbufferReceive(
        grid_esp32_adc_state.ringbuffer_handle, &adc_result_size, 0);
    ...
    if (adc_result != NULL) {
      ...
      grid_ui_potmeter_store_input(multiplexer_lookup[lookup_index],
    ...
    result = (struct grid_esp32_encoder_result *)xRingbufferReceive(
        grid_esp32_encoder_state.ringbuffer_handle, &size, 0);
    ...
    if (result != NULL) {
      ...
      for (uint8_t j = 0; j < 4; j++) {
        ...
        grid_ui_encoder_store_input(i, &encoder_last_real_time[i],
                                    &button_last_real_time[i], old_value,
                                    new_value, &phase_change_lock_array[i]);
      }
