...
void grid_d51_uart_init() {
  ...
  usart_async_register_callback(&USART_NORTH, USART_ASYNC_TXC_CB,
                                tx_cb_USART_GRID_N);
  usart_async_register_callback(&USART_EAST, USART_ASYNC_TXC_CB,
                                tx_cb_USART_GRID_E);
  usart_async_register_callback(&USART_SOUTH, USART_ASYNC_TXC_CB,
                                tx_cb_USART_GRID_S);
  usart_async_register_callback(&USART_WEST, USART_ASYNC_TXC_CB,
                                tx_cb_USART_GRID_W);
...
void grid_d51_uart_dma_rx_init_one(struct usart_async_descriptor *usart,
  ...
  _dma_set_source_address(
      dma_rx_channel,
      (uint32_t) & (((Sercom *)((*usart).device.hw))->USART.DATA.reg));
...
void grid_d51_uart_dma_rx_init() {
  ...
  grid_d51_uart_dma_rx_init_one(
      &USART_NORTH, DMA_NORTH_RX_CHANNEL, uart_port_array[0]->rx_double_buffer,
      GRID_DOUBLE_BUFFER_RX_SIZE, dma_transfer_complete_n_cb);
  grid_d51_uart_dma_rx_init_one(
      &USART_EAST, DMA_EAST_RX_CHANNEL, uart_port_array[1]->rx_double_buffer,
      GRID_DOUBLE_BUFFER_RX_SIZE, dma_transfer_complete_e_cb);
  grid_d51_uart_dma_rx_init_one(
      &USART_SOUTH, DMA_SOUTH_RX_CHANNEL, uart_port_array[2]->rx_double_buffer,
      GRID_DOUBLE_BUFFER_RX_SIZE, dma_transfer_complete_s_cb);
  grid_d51_uart_dma_rx_init_one(
      &USART_WEST, DMA_WEST_RX_CHANNEL, uart_port_array[3]->rx_double_buffer,
      GRID_DOUBLE_BUFFER_RX_SIZE, dma_transfer_complete_w_cb);
