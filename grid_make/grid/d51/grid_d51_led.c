...
void grid_d51_led_init(struct grid_d51_led_model *d51_mod,
  ...
  //
  //  * Reading intensity values from lookup table reduces the CPU cycles used
  //  for calculating the fram buffer.
  //  * This is especially useful for fast animations using large number of
  //  LEDs.
  //
  ...
  //  * Green Red and Blue intensity values are sent to the LEDs in the given
  //  order.
  //  * The output should have a baudrate of 3.2Mbps resulting in an effective
  //  symbol rate of 800k sym per sec
  //
  ...
  for (uint16_t i = 0; i < 256; i++) {
    ...
    temp |=
        (i / 1 % 2) ? (GRID_D51_LED_CODE_O << 24) : (GRID_D51_LED_CODE_Z << 24);
    temp |=
        (i / 2 % 2) ? (GRID_D51_LED_CODE_O << 28) : (GRID_D51_LED_CODE_Z << 28);

    temp |=
        (i / 4 % 2) ? (GRID_D51_LED_CODE_O << 16) : (GRID_D51_LED_CODE_Z << 16);
    temp |=
        (i / 8 % 2) ? (GRID_D51_LED_CODE_O << 20) : (GRID_D51_LED_CODE_Z << 20);

    temp |=
        (i / 16 % 2) ? (GRID_D51_LED_CODE_O << 8) : (GRID_D51_LED_CODE_Z << 8);
    temp |= (i / 32 % 2) ? (GRID_D51_LED_CODE_O << 12)
                         : (GRID_D51_LED_CODE_Z << 12);
    temp |=
        (i / 64 % 2) ? (GRID_D51_LED_CODE_O << 0) : (GRID_D51_LED_CODE_Z << 0);
    temp |=
        (i / 128 % 2) ? (GRID_D51_LED_CODE_O << 4) : (GRID_D51_LED_CODE_Z << 4);
  ...
  d51_mod->framebuffer_size =
      (GRID_D51_LED_RESET_LENGTH + led_mod->led_count * 3 * 4);
  d51_mod->framebuffer =
      (uint8_t *)malloc(d51_mod->framebuffer_size * sizeof(uint8_t));

  // Fill the first 24 bytes with the rr_code (reset)
  // This memory is essentially wasted but allows the entire frame to be sent
  // out using DMA
...
void grid_d51_led_set_color(struct grid_d51_led_model *mod, uint32_t led_index,
  ...
  uint32_t *framebuffer_usable =
      (uint32_t *)&mod->framebuffer[GRID_D51_LED_RESET_LENGTH];
...
void grid_led_startup_animation(struct grid_d51_led_model *mod) {
  ...
  for (uint8_t i = 0; i < 255; i++) {
    ...
    for (uint8_t j = 0; j < mod->led_count; j++) {
      ...
      grid_d51_led_set_color(
          mod, j, (color_r * i * s % 256) / 2, (color_g * i * s % 256) / 2,
          (color_b * i * s % 256) /
              2); // This is not an alert, this is low level shit
    }
