...
void grid_ui_model_init(struct grid_ui_model *mod,
  ...
  mod->element_list =
      malloc(element_list_length * sizeof(struct grid_ui_element));
...
void grid_ui_element_init(struct grid_ui_model *parent, uint8_t index,
                          enum grid_ui_element_t element_type) {
  ...
  if (element_type == GRID_UI_ELEMENT_SYSTEM) {
    ...
    ele->event_list =
        malloc(ele->event_list_length * sizeof(struct grid_ui_event));
    ...
    ele->template_initializer =
        &grid_ui_element_potmeter_template_parameter_init;
    ...
    ele->template_initializer =
        &grid_ui_element_encoder_template_parameter_init;
...
void grid_ui_event_init(struct grid_ui_element *ele, uint8_t index,
                        enum grid_ui_event_t event_type) {
    ...
    if (ele->timer_source_is_midi == 0) {
      ...
      if (ele->timer_event_helper > 0) {
        ...
        if (ele->timer_event_helper == 0) {
          ...
          struct grid_ui_event *eve =
              grid_ui_event_find(ele, GRID_UI_EVENT_TIMER);
...
void grid_ui_rtc_ms_mapmode_handler(struct grid_ui_model *ui,
                                    uint8_t new_mapmode_value) {
  ...
  if (new_mapmode_value != ui->mapmode_state) {
    ...
    if (ui->mapmode_state == 0) { // RELEASE
      ...
      struct grid_ui_element *sys_ele =
          &grid_ui_state.element_list[grid_ui_state.element_list_length - 1];

      struct grid_ui_event *eve =
          grid_ui_event_find(sys_ele, GRID_UI_EVENT_MAPMODE_CHANGE);
...
grid_ui_template_buffer_create(struct grid_ui_element *ele) {
...
struct grid_ui_template_buffer *
grid_ui_template_buffer_find(struct grid_ui_element *ele, uint8_t page) {
...
void grid_ui_page_load(struct grid_ui_model *ui, uint8_t page) {
  ...
  */
  ...
  for (uint8_t i = 0; i < ui->element_list_length; i++) {
    ...
    // page==0?1:page);
    struct grid_ui_template_buffer *buf =
        grid_ui_template_buffer_find(ele, page);
...
struct grid_ui_event *grid_ui_event_find(struct grid_ui_element *ele,
                                         enum grid_ui_event_t event_type) {
...
uint8_t grid_ui_event_isdefault_actionstring(struct grid_ui_event *eve,
                                             char *action_string) {
  ...
  if (ele->type == GRID_UI_ELEMENT_SYSTEM &&
      eve->type == GRID_UI_EVENT_MAPMODE_CHANGE) {
    return (strcmp(action_string, GRID_ACTIONSTRING_MAPMODE_CHANGE) == 0);
  }

  // SYSTEM - MIDIRX
  if (ele->type == GRID_UI_ELEMENT_SYSTEM &&
      eve->type == GRID_UI_EVENT_MIDIRX) {
...
void grid_ui_event_register_actionstring(struct grid_ui_event *eve,
                                         char *action_string) {
  ...
  sprintf(temp, "ele[%d].%s = function (self) %s end", ele->index,
          eve->function_name, &action_string[6]);
  action_string[len - 3] = ' ';
  ...
  if (0 == grid_lua_dostring(&grid_lua_state, temp)) {
    grid_port_debug_printf(
        "LUA not OK, Failed to register action! EL: %d EV: %d", ele->index,
        eve->type);
  };
...
uint32_t grid_ui_event_render_event(struct grid_ui_event *eve,
                                    char *target_string) {
  ...
  if (eve->parent->type == GRID_UI_ELEMENT_POTENTIOMETER) {
    param = eve->parent
                ->template_parameter_list[GRID_LUA_FNC_P_POTMETER_STATE_index];
  } else if (eve->parent->type == GRID_UI_ELEMENT_BUTTON) {
    param =
        eve->parent->template_parameter_list[GRID_LUA_FNC_B_BUTTON_STATE_index];
  } else if (eve->parent->type == GRID_UI_ELEMENT_ENCODER &&
             eve->type == GRID_UI_EVENT_EC) {
    param = eve->parent
                ->template_parameter_list[GRID_LUA_FNC_E_ENCODER_STATE_index];
  } else if (eve->parent->type == GRID_UI_ELEMENT_ENCODER &&
             eve->type == GRID_UI_EVENT_BC) {
    param =
        eve->parent->template_parameter_list[GRID_LUA_FNC_E_BUTTON_STATE_index];
  }
  ...
  grid_msg_string_set_parameter(target_string, GRID_INSTR_offset,
                                GRID_INSTR_length, GRID_INSTR_EXECUTE_code,
                                NULL);

  grid_msg_string_set_parameter(target_string,
                                GRID_CLASS_EVENT_PAGENUMBER_offset,
                                GRID_CLASS_EVENT_PAGENUMBER_length, page, NULL);
  grid_msg_string_set_parameter(
      target_string, GRID_CLASS_EVENT_ELEMENTNUMBER_offset,
      GRID_CLASS_EVENT_ELEMENTNUMBER_length, element, NULL);
  grid_msg_string_set_parameter(target_string,
                                GRID_CLASS_EVENT_EVENTTYPE_offset,
                                GRID_CLASS_EVENT_EVENTTYPE_length, event, NULL);
  grid_msg_string_set_parameter(
      target_string, GRID_CLASS_EVENT_EVENTPARAM_offset,
      GRID_CLASS_EVENT_EVENTPARAM_length, param, NULL);

  return strlen(target_string);
}

void grid_ui_event_generate_actionstring(struct grid_ui_event *eve,
                                         char *targetstring) {
...
void grid_ui_event_get_actionstring(struct grid_ui_event *eve,
                                    char *targetstring) {
  ...
  sprintf(temp, "gsg(%ld,debug.getinfo(ele[%d].%s,\"S\").source)",
          (uint32_t)result, eve->parent->index, eve->function_name);

  // grid_platform_printf("TO  %x -> %s\r\n", result, result);

  if (0 == grid_lua_dostring(&grid_lua_state, temp)) {
    grid_port_debug_printf(
        "LUA not OK, Failed to retrieve action! EL: %d EV: %d",
...
uint32_t grid_ui_event_render_action(struct grid_ui_event *eve,
                                     char *target_string) {
  ...
  sprintf(temp, "<?lua ele[%d]:%s(self) ?>", eve->parent->index,
          eve->function_name);
    ...
    if (0 == strncmp(&temp[i], "<?lua ", 6)) {
      ...
      if (code_type == 2) { // LUA
        ...
        if (0 == grid_lua_dostring(&grid_lua_state, &temp[code_start + 6])) {
          grid_port_debug_printf("LUA not OK! EL: %d EV: %d",
                                 eve->parent->index, eve->type);
        };

        uint32_t code_stdo_length =
            strlen(grid_lua_get_output_string(&grid_lua_state));
        ...
        strcpy(&target_string[code_start - total_substituted_length],
               grid_lua_get_output_string(&grid_lua_state));
        ...
        if (strlen(grid_lua_get_error_string(&grid_lua_state))) {
          ...
          sprintf(&errorbuffer[strlen(errorbuffer)],
                  GRID_CLASS_DEBUGTEXT_frame_end);
...
void grid_ui_event_recall_configuration(struct grid_ui_model *ui, uint8_t page,
                                        uint8_t element,
                                        enum grid_ui_event_t event_type,
                                        char *targetstring) {
...
struct grid_ui_element *grid_ui_element_find(struct grid_ui_model *ui,
...
void grid_ui_element_set_template_parameter(struct grid_ui_element *ele,
...
int32_t grid_ui_element_get_template_parameter(struct grid_ui_element *ele,
                                               uint8_t template_index) {
...
void grid_ui_element_potmeter_template_parameter_init(
    struct grid_ui_template_buffer *buf) {
  ...
  int32_t resolution =
      template_parameter_list[GRID_LUA_FNC_P_POTMETER_MODE_index];
  ...
  int32_t next = grid_ain_get_average_scaled(&grid_ain_state, element_index, 16,
                                             resolution, min, max);
  template_parameter_list[GRID_LUA_FNC_P_POTMETER_VALUE_index] = next;
}

void grid_ui_element_button_template_parameter_init(
    struct grid_ui_template_buffer *buf) {
...
void grid_ui_element_encoder_template_parameter_init(
    struct grid_ui_template_buffer *buf) {
...
void grid_ui_element_button_page_change_cb(struct grid_ui_element *ele,
                                           uint8_t page_old, uint8_t page_new) {
...
void grid_ui_element_encoder_event_clear_cb(struct grid_ui_event *eve) {
  ...
  if (template_parameter_list[GRID_LUA_FNC_E_ENCODER_MODE_index] ==
      1) { // relative
    ...
    template_parameter_list[GRID_LUA_FNC_E_ENCODER_VALUE_index] =
        ((max + 1) - min) / 2;
...
void grid_ui_element_encoder_page_change_cb(struct grid_ui_element *ele,
                                            uint8_t page_old,
                                            uint8_t page_new) {
...
void grid_ui_element_potmeter_page_change_cb(struct grid_ui_element *ele,
                                             uint8_t page_old,
                                             uint8_t page_new) {
  ...
  int32_t next = grid_ain_get_average_scaled(&grid_ain_state, element_index,
                                             grid_platform_get_adc_bit_depth(),
                                             resolution, min, max);
  template_parameter_list[GRID_LUA_FNC_P_POTMETER_VALUE_index] = next;
}
  ...
  for (uint8_t i = last_element_helper; i < ui->element_list_length; i++) {
    ...
    for (uint8_t j = 0; j < ele->event_list_length; j++) {
      ...
      if (firstrun) {
        ...
        if (j >= ele->event_list_length) { // to check last_event_helper in the
                                           // first iteration
          break;
        }
      }
    ...
    for (uint8_t j = 0; j < ele->event_list_length; j++) {
      ...
      if (eve->cfg_changed_flag) {
        ...
        if (eve->cfg_default_flag) {
          ...
          grid_platform_write_actionstring_file(ele->parent->page_activepage,
                                                ele->index, eve->type, buffer,
                                                strlen(buffer));
        }
...
void grid_ui_bulk_nvmerase_next(struct grid_ui_model *ui) {
  ...
  // flash_erase takes 32 ms so no need to implement timeout here, only erase
  // one block at a time!
...
void grid_port_process_ui_local_UNSAFE(struct grid_ui_model *ui) {
  ...
  grid_msg_packet_init(&grid_msg_state, &message_local,
  ...
  grid_msg_packet_init(&grid_msg_state, &message_global,
                       GRID_PARAMETER_DEFAULT_POSITION,
                       GRID_PARAMETER_DEFAULT_POSITION);
  ...
  for (uint8_t j = 0; j < ui->element_list_length; j++) {
    ...
    for (uint8_t k = 0; k < ele->event_list_length; k++) {
      ...
      if (offset_local > GRID_PARAMETER_PACKET_marign ||
          offset_global > GRID_PARAMETER_PACKET_marign) {
        continue;
      } else {
        ...
        if (grid_ui_event_istriggered_local(eve)) {
          ...
          offset_local +=
              grid_ui_event_render_action(eve, &payload_local[offset_local]);
          grid_ui_event_reset(eve);
...
void grid_port_process_ui_UNSAFE(struct grid_ui_model *ui) {
    ...
    for (uint8_t k = 0; k < ele->event_list_length; k++) {
      ...
      if (grid_msg_packet_get_length(&message) > GRID_PARAMETER_PACKET_marign) {
        ...
        if (grid_ui_event_istriggered(eve)) {
          ...
          message.body_length +=
              grid_ui_event_render_event(eve, &message.body[offset]);

          offset = grid_msg_packet_body_get_length(&message);

          message.body_length +=
              grid_ui_event_render_action(eve, &message.body[offset]);
          grid_ui_event_reset(eve);
  ...
  if (grid_protocol_led_change_report_length(&grid_led_state) &&
      grid_sys_get_editor_connected_state(&grid_sys_state)) {
