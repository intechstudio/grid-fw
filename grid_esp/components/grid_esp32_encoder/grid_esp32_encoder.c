...
void grid_esp32_encoder_spi_init(struct grid_esp32_encoder_model *encoder,
  ...
  esp_err_t ret =
      spi_bus_initialize(encoder->spi_host, &buscfg, SPI_DMA_CH_AUTO);
  ESP_ERROR_CHECK(ret);

  ret = spi_bus_add_device(encoder->spi_host, &devcfg,
                           &encoder->spi_device_handle);
  ESP_ERROR_CHECK(ret);
...
void grid_esp32_encoder_init(struct grid_esp32_encoder_model *encoder,
  ...
  encoder->rx_buffer_previous =
      (uint8_t *)malloc(GRID_ESP32_ENCODER_BUFFER_SIZE * sizeof(uint8_t));
  encoder->rx_buffer =
      (uint8_t *)malloc(GRID_ESP32_ENCODER_BUFFER_SIZE * sizeof(uint8_t));
  encoder->tx_buffer =
      (uint8_t *)malloc(GRID_ESP32_ENCODER_BUFFER_SIZE * sizeof(uint8_t));

  memset(encoder->rx_buffer_previous, 0,
         GRID_ESP32_ENCODER_BUFFER_SIZE * sizeof(uint8_t));
  memset(encoder->rx_buffer, 0,
         GRID_ESP32_ENCODER_BUFFER_SIZE * sizeof(uint8_t));
  memset(encoder->tx_buffer, 0,
         GRID_ESP32_ENCODER_BUFFER_SIZE * sizeof(uint8_t));
  ...
  encoder->buffer_struct = (StaticRingbuffer_t *)heap_caps_malloc(
      sizeof(StaticRingbuffer_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  encoder->buffer_storage =
      (struct grid_esp32_encoder_result *)heap_caps_malloc(
          sizeof(struct grid_esp32_encoder_result) * ENCODER_BUFFER_SIZE,
          MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  encoder->ringbuffer_handle =
      xRingbufferCreateStatic(ENCODER_BUFFER_SIZE, ENCODER_BUFFER_TYPE,
                              encoder->buffer_storage, encoder->buffer_struct);

  return;
}

static bool IRAM_ATTR example_timer_on_alarm_cb(
    gptimer_handle_t timer, const gptimer_alarm_event_data_t *edata,
    void *user_ctx) {
  BaseType_t high_task_awoken = pdFALSE;

  struct grid_esp32_encoder_model *encoder =
      (struct grid_esp32_encoder_model *)user_ctx;
...
void grid_esp32_encoder_start(struct grid_esp32_encoder_model *encoder) {
  ...
  ESP_ERROR_CHECK(
      gptimer_register_event_callbacks(gptimer, &cbs, (void *)encoder));
...
void IRAM_ATTR grid_esp32_encoder_spi_start_transfer(
    struct grid_esp32_encoder_model *encoder) {
