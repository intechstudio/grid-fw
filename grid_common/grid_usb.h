...
extern int32_t grid_platform_usb_midi_write(uint8_t byte0, uint8_t byte1,
                                            uint8_t byte2, uint8_t byte3);
extern int32_t grid_platform_usb_midi_write_status(void);
extern int32_t grid_platform_usb_mouse_button_change(uint8_t b_state,
                                                     uint8_t type);
...
enum grid_usb_keyboard_key_state_t {
  GRID_USB_KEYBOARD_KEY_STATEUP,
  GRID_USB_KEYBOARD_KEY_STATEDOWN
};
...
extern struct grid_midi_event_desc
    grid_midi_tx_buffer[GRID_MIDI_TX_BUFFER_length];
...
extern struct grid_midi_event_desc
    grid_midi_rx_buffer[GRID_MIDI_RX_BUFFER_length];
...
struct grid_usb_keyboard_model {
  ...
  struct grid_usb_keyboard_event_desc
      active_key_list[GRID_KEYBOARD_KEY_maxcount];
