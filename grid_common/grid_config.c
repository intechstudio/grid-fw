...
int grid_config_parse_cal(struct grid_cal_model* cal, char* errbuf, toml_table_t* conf) {
  ...
  for (int i = 0; i < cal->length; ++i) {
    ...
    if (ctr.ok) {
      ...
      int status = grid_cal_channel_get(cal, i, GRID_CAL_CENTER, (void**)&center);
      if (status) {
        return status;
      }
      if (center) {
  ...
  for (int i = 0; i < cal->length; ++i) {
    ...
    if (min.ok) {
      ...
      int status = grid_cal_channel_get(cal, i, GRID_CAL_LIMITS, (void**)&limits);
      if (status) {
        return status;
      }
      if (limits) {
  ...
  for (int i = 0; i < cal->length; ++i) {
    ...
    if (det.ok) {
      ...
      int status = grid_cal_channel_get(cal, i, GRID_CAL_DETENT, (void**)&detent);
      if (status) {
        return status;
      }
      if (detent) {
