...
// 			
// 			grid_nvm_state.read_buffer[511] = '\n';
// 			
// 			for (uint16_t i=0; i<512; i++)
// 			{
// 				message[i] = grid_nvm_state.read_buffer[i] ;
// 				
// 			}
// 
// 			cdcdf_acm_write(message , 512);
// 			
// 			delay_ms(10);
	...
	while (1) {
		...
		if (usb_init_variable == 1 && grid_sys_rtc_get_time(&grid_sys_state)>RTC1SEC*4){
			...
			if (debug_flag == 0){
				...
				grid_sys_alert_set_alert(&grid_sys_state, 255, 255, 255, 0, 500); // GREEN	
		...
		if (debug_flag == 1){
			...
			for(uint8_t i=0; i<grid_ui_state.element[element].event_list_length; i++){
				...
				for (uint8_t j=0; j<grid_ui_state.element[element].event_list[i].action_string_length; j++)
				{
