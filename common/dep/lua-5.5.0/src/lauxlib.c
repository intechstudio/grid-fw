...
static const char *getF (lua_State *L, void *ud, size_t *size) {
  ...
  if (lf->n > 0) {  /* are there pre-read characters to be read? */
    ...
    *size = l_fread(lf->buff, 1, sizeof(lf->buff), lf->f);
...
static int skipBOM (FILE *f) {
  int c = l_getc(f);  /* read first character */
  if (c == 0xEF && l_getc(f) == 0xBB && l_getc(f) == 0xBF)  /* correct BOM? */
    return l_getc(f);  /* ignore BOM and return next char */
  else  /* no (valid) BOM */
...
static int skipcomment (FILE *f, int *cp) {
  ...
  if (c == '#') {  /* first line is a comment (Unix exec. file)? */
    ...
    *cp = l_getc(f);  /* next character after comment, if present */
