...
uint8_t grid_ain_channel_init(struct grid_ain_model *mod, uint8_t channel,
  ...
  mod->channel_buffer[channel].buffer =
      malloc(mod->channel_buffer[channel].buffer_depth * sizeof(uint16_t));
...
uint8_t grid_ain_init(struct grid_ain_model *mod, uint8_t length,
  ...
  mod->channel_buffer =
      (struct AIN_Channel *)malloc(length * sizeof(struct AIN_Channel));
...
uint8_t grid_ain_add_sample(struct grid_ain_model *mod, uint8_t channel,
  ...
  uint8_t criteria_b = grid_ain_abs(instance->result_average - average) >
                       (1 << downscale_factor);
  uint8_t criteria_c =
      upscaled > ((1 << source_resolution) - (1 << upscale_factor) - 1);
