	...
	for(uint8_t i=0; i<mod->led_count; i++){
		...
		if (mod->led_changed_flag_array[i] != 0){
			...
			if (length + 8 <= maxlength){
				...
				grid_msg_set_parameter(&output[length], 2, 2, mod->led_frame_buffer[i*3 + 1], NULL);
				grid_msg_set_parameter(&output[length], 4, 2, mod->led_frame_buffer[i*3 + 0], NULL);
				grid_msg_set_parameter(&output[length], 6, 2, mod->led_frame_buffer[i*3 + 2], NULL);
