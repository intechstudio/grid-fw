...
size_t vmp_evt_serialize(void* evt, uint8_t* dest) {
  ...
  *((uint16_t*)(dest + size)) = htons((event)->scln);
  size += sizeof(uint16_t);
...
size_t vmp_evt_deserialize(uint8_t* src, void* evt) {
  ...
  size += sizeof(uint32_t);
  event->scln = ntohs(*((uint16_t*)(src + size)));
  size += sizeof(uint16_t);
...
size_t vmp_evt_serialized_size() {
  return sizeof(uint16_t) + // uid
         ...
         sizeof(uint16_t) + // scanline
         0;
}

extern void grid_platform_delay_us(uint32_t delay_microseconds);
