...
void tud_midi_rx_cb(uint8_t itf) {
  ...
  while (tud_midi_available()) {
    ...
    if (tud_midi_packet_read(packet)) {
      // Pass raw USB MIDI packet bytes to the state machine.
      // Byte1 contains the combined status byte (channel | command);
      // channel/command decoding happens in grid_midi_rx_pop().
