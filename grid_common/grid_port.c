...
static void grid_port_timeout_try_disconect(struct grid_port *por) {
  ...
  if (grid_platform_rtc_get_elapsed_time(por->rx_double_buffer_timestamp) <
      1000 * 1000) {
  ...
  if (por->partner_status == 1) {
    ...
    if (por->rx_double_buffer_read_start_index == 0 &&
      ...
      grid_platform_printf("Timeout Disconnect 2 %c (R%d S%d W%d)\r\n",
                           grid_port_get_name_char(por),
                           por->rx_double_buffer_read_start_index,
...
static uint8_t grid_port_rxdobulebuffer_check_overrun(struct grid_port *por) {
  ...
  uint8_t overrun_condition_1 = (por->rx_double_buffer_seek_start_index ==
                                 por->rx_double_buffer_read_start_index - 1);
  uint8_t overrun_condition_2 = (por->rx_double_buffer_seek_start_index ==
                                     GRID_DOUBLE_BUFFER_RX_SIZE - 1 &&
                                 por->rx_double_buffer_read_start_index == 0);
  uint8_t overrun_condition_3 =
      (por->rx_double_buffer[(por->rx_double_buffer_read_start_index +
                              GRID_DOUBLE_BUFFER_RX_SIZE - 1) %
                             GRID_DOUBLE_BUFFER_RX_SIZE] != 0);
...
static void grid_port_rxdobulebuffer_seek_newline(struct grid_port *por) {
  ...
  for (uint16_t i = 0; i < 490; i++) { // 490 is the max processing length
    ...
    if (por->rx_double_buffer[por->rx_double_buffer_seek_start_index] ==
      ...
      break;
    } else if (por->rx_double_buffer[por->rx_double_buffer_seek_start_index] ==
               0) {
    ...
    if (por->rx_double_buffer_seek_start_index <
        GRID_DOUBLE_BUFFER_RX_SIZE - 1) {
...
void grid_port_receive_task(struct grid_port *por) {
  ...
  if (por->rx_double_buffer_read_start_index <
      por->rx_double_buffer_seek_start_index) {
    length = por->rx_double_buffer_seek_start_index -
             por->rx_double_buffer_read_start_index + 1;
  } else {
    length = GRID_DOUBLE_BUFFER_RX_SIZE +
             por->rx_double_buffer_seek_start_index -
             por->rx_double_buffer_read_start_index + 1;
  }
...
uint8_t grid_msg_is_position_transformable(int8_t received_x,
                                           int8_t received_y) {
  // Position is transformabe if x and y positions do not indicate global
  // message or editor message

  if (received_x + GRID_PARAMETER_DEFAULT_POSITION == 0 &&
      received_y + GRID_PARAMETER_DEFAULT_POSITION == 0) {
    // EDITOR GENERATED GLOBAL MESSAGE
    return false;

  } else if (received_x + GRID_PARAMETER_DEFAULT_POSITION == 255 &&
             received_y + GRID_PARAMETER_DEFAULT_POSITION == 255) {
...
void grid_msg_string_transform_brc_params(char *message, int8_t dx, int8_t dy,
                                          uint8_t partner_fi) {

  uint8_t error = 0;

  uint8_t received_session = grid_msg_string_get_parameter(
      message, GRID_BRC_SESSION_offset, GRID_BRC_SESSION_length, &error);
  uint8_t received_msgage = grid_msg_string_get_parameter(
      message, GRID_BRC_MSGAGE_offset, GRID_BRC_MSGAGE_length, &error);

  // Read the received destination X Y values (SIGNED INT)
  int8_t received_dx =
      grid_msg_string_get_parameter(message, GRID_BRC_DX_offset,
                                    GRID_BRC_DX_length, &error) -
      GRID_PARAMETER_DEFAULT_POSITION;
  int8_t received_dy =
      grid_msg_string_get_parameter(message, GRID_BRC_DY_offset,
                                    GRID_BRC_DY_length, &error) -
      GRID_PARAMETER_DEFAULT_POSITION;

  // Read the received source X Y values (SIGNED INT)
  int8_t received_sx =
      grid_msg_string_get_parameter(message, GRID_BRC_SX_offset,
                                    GRID_BRC_SX_length, &error) -
      GRID_PARAMETER_DEFAULT_POSITION;
  int8_t received_sy =
      grid_msg_string_get_parameter(message, GRID_BRC_SY_offset,
                                    GRID_BRC_SY_length, &error) -
      GRID_PARAMETER_DEFAULT_POSITION;

  uint8_t received_rot = grid_msg_string_get_parameter(
      message, GRID_BRC_ROT_offset, GRID_BRC_ROT_length, &error);
  ...
  if (grid_msg_is_position_transformable(received_dx, received_dy)) {
    ...
    grid_msg_string_set_parameter(message, GRID_BRC_DX_offset,
                                  GRID_BRC_DX_length, updated_dx, &error);
    grid_msg_string_set_parameter(message, GRID_BRC_DY_offset,
                                  GRID_BRC_DY_length, updated_dy, &error);
  }

  if (grid_msg_is_position_transformable(received_sx, received_sy)) {

    // Update message with the new values
    grid_msg_string_set_parameter(message, GRID_BRC_SX_offset,
                                  GRID_BRC_SX_length, updated_sx, &error);
    grid_msg_string_set_parameter(message, GRID_BRC_SY_offset,
                                  GRID_BRC_SY_length, updated_sy, &error);
  }

  grid_msg_string_set_parameter(message, GRID_BRC_MSGAGE_offset,
                                GRID_BRC_MSGAGE_length, updated_msgage, &error);
  grid_msg_string_set_parameter(message, GRID_BRC_ROT_offset,
                                GRID_BRC_ROT_length, updated_rot, &error);
  grid_msg_string_set_parameter(message, GRID_BRC_PORTROT_offset,
                                GRID_BRC_PORTROT_length, partner_fi, &error);

  // Recalculate and update the checksum
  uint16_t length = strlen(message);
  grid_msg_string_checksum_write(
      message, length,
      grid_msg_string_calculate_checksum_of_packet_string(message, length));
}
...
uint32_t grid_msg_recent_fingerprint_calculate(char *message) {
  ...
  uint8_t received_id = grid_msg_string_get_parameter(
      message, GRID_BRC_ID_offset, GRID_BRC_ID_length, &error);
  uint8_t received_session = grid_msg_string_get_parameter(
      message, GRID_BRC_SESSION_offset, GRID_BRC_SESSION_length, &error);
  int8_t updated_sx =
      grid_msg_string_get_parameter(message, GRID_BRC_SX_offset,
  ...
  uint32_t fingerprint = received_id * 256 * 256 * 256 +
                         updated_sx * 256 * 256 + updated_sy * 256 +
                         received_session;

  return fingerprint;
}

static void grid_port_rxdobulebuffer_receive_to_buffer(struct grid_port *por,
                                                       char *buffer,
                                                       uint16_t length) {
  ...
  for (uint16_t i = 0; i < length; i++) {
    buffer[i] =
        por->rx_double_buffer[(por->rx_double_buffer_read_start_index + i) %
                              GRID_DOUBLE_BUFFER_RX_SIZE];
    por->rx_double_buffer[(por->rx_double_buffer_read_start_index + i) %
                          GRID_DOUBLE_BUFFER_RX_SIZE] = 0;
  }
  buffer[length] = 0;
  ...
  por->rx_double_buffer_read_start_index =
      (por->rx_double_buffer_read_start_index + length) %
      GRID_DOUBLE_BUFFER_RX_SIZE;
  por->rx_double_buffer_seek_start_index =
      por->rx_double_buffer_read_start_index;
...
void grid_port_receive_broadcast_message(struct grid_port *por, char *message,
                                         uint16_t length) {

  uint8_t error = 0;

  // update age, sx, sy, dx, dy, rot etc...
  grid_msg_string_transform_brc_params(message, por->dx, por->dy,
                                       por->partner_fi);
...
void grid_port_receive_direct_message(struct grid_port *por, char *message,
                                      uint16_t length) {
  ...
  if (message[2] == GRID_CONST_BELL) {
    ...
    if (por->partner_status == 0) {
      ...
      por->partner_hwcfg = grid_msg_string_read_hex_string_value(
          &message[length - 10], 2, &error);
...
void grid_port_receive_decode(struct grid_port *por, uint16_t len) {
  ...
  if (checksum_calculated != checksum_received) {
    ...
    uint16_t packet_length = grid_msg_string_get_parameter(
        message, GRID_BRC_LEN_offset, GRID_BRC_LEN_length, &error);
    grid_platform_printf("##CHK %d %d\r\n", packet_length, length);
    grid_port_debug_printf("Checksum %02x %02x", checksum_calculated,
                           checksum_received);
    return;
  }
...
struct grid_port *grid_port_allocate(void) {
  ...
  struct grid_port *por = (struct grid_port *)grid_platform_allocate_volatile(
      sizeof(struct grid_port));

  por->tx_buffer.buffer_storage =
      (char *)grid_platform_allocate_volatile(GRID_BUFFER_SIZE);
  memset(por->tx_buffer.buffer_storage, 0, GRID_BUFFER_SIZE);

  por->rx_buffer.buffer_storage =
      (char *)grid_platform_allocate_volatile(GRID_BUFFER_SIZE);
...
void grid_port_init(struct grid_port *por, uint8_t type, uint8_t dir,
  ...
  if (type == GRID_PORT_TYPE_USART) {
    ...
    sprintf((char *)por->ping_packet, "%c%c%c%c%02lx%02x%02x%c00\n",
            GRID_CONST_SOH, GRID_CONST_DCT, GRID_CONST_BELL, por->direction,
            grid_sys_get_hwcfg(&grid_sys_state), 255, 255, GRID_CONST_EOT);

    por->ping_packet_length = strlen((char *)por->ping_packet);

    grid_msg_string_checksum_write(
        por->ping_packet, por->ping_packet_length,
    ...
    por->partner_status =
        1; // UI AND USB are considered to be connected by default
  }
}
...
uint8_t grid_port_process_outbound_usart(struct grid_port *por) {
  ...
  grid_platform_send_grid_message(por->direction, por->tx_double_buffer,
                                  packet_size);
...
uint8_t grid_port_process_outbound_usb(volatile struct grid_port *por) {
  ...
  for (uint16_t i = 0; i < length; i++) {
    ...
    uint8_t msg_class =
        grid_msg_string_get_parameter(chunk, GRID_PARAMETER_CLASSCODE_offset,
                                      GRID_PARAMETER_CLASSCODE_length, &error);
    uint8_t msg_instr = grid_msg_string_get_parameter(
        chunk, GRID_INSTR_offset, GRID_INSTR_length, &error);
...
void grid_port_receiver_hardreset(struct grid_port *por) {
  ...
  grid_msg_string_write_hex_string_value(&por->ping_packet[8], 2,
                                         por->ping_partner_token);
  grid_msg_string_write_hex_string_value(&por->ping_packet[6], 2,
                                         por->ping_local_token);
  grid_msg_string_checksum_write(
      por->ping_packet, por->ping_packet_length,
      grid_msg_string_calculate_checksum_of_packet_string(
          por->ping_packet, por->ping_packet_length));
...
void grid_port_debug_print_text(char *debug_string) {
  ...
  grid_msg_packet_body_append_parameter(
      &message, GRID_INSTR_offset, GRID_INSTR_length, GRID_INSTR_EXECUTE_code);
...
void grid_port_websocket_print_text(char *debug_string) {
  ...
  grid_msg_packet_body_append_printf(&message,
                                     GRID_CLASS_WEBSOCKET_frame_start);
  grid_msg_packet_body_append_parameter(
      &message, GRID_INSTR_offset, GRID_INSTR_length, GRID_INSTR_EXECUTE_code);
  grid_msg_packet_body_append_printf(&message, debug_string);
...
void grid_port_ping_try_everywhere(void) {
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    if (grid_buffer_write_size(&next_port->tx_buffer) <
        next_port->ping_packet_length) {
      // not enough space in buffer!
      continue;
    }

    grid_buffer_write_from_chunk(&next_port->tx_buffer, next_port->ping_packet,
                                 next_port->ping_packet_length);
...
void grid_protocol_nvm_erase_succcess_callback() {
  ...
  uint8_t lastheader_id = grid_msg_get_lastheader_id(
  ...
  grid_msg_packet_body_append_parameter(
      ...
      GRID_CLASS_NVMERASE_LASTHEADER_length, lastheader_id);
  ...
  grid_ui_page_load(&grid_ui_state,
                    grid_ui_page_get_activepage(&grid_ui_state));
}
...
void grid_protocol_nvm_clear_succcess_callback() {
  ...
  grid_msg_packet_body_append_parameter(
      ...
      GRID_CLASS_PAGECLEAR_LASTHEADER_length, lastheader_id);
...
void grid_protocol_nvm_read_succcess_callback() {
  ...
  grid_msg_packet_body_append_parameter(
      ...
      GRID_CLASS_PAGEDISCARD_LASTHEADER_length, lastheader_id);
...
void grid_protocol_nvm_store_succcess_callback() {
  ...
  grid_msg_packet_body_append_parameter(
      ...
      GRID_CLASS_PAGESTORE_LASTHEADER_length, lastheader_id);
  ...
  grid_msg_packet_body_append_parameter(
      &response, GRID_INSTR_offset, GRID_INSTR_length, GRID_INSTR_EXECUTE_code);
  grid_msg_packet_body_append_printf(&response, "xxstore complete offset 0x%x",
                                     grid_plaform_get_nvm_nextwriteoffset());
  grid_msg_packet_body_append_printf(&response, GRID_CLASS_DEBUGTEXT_frame_end);
...
void grid_protocol_send_heartbeat() {
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    portstate |=
        ((grid_transport_get_port(&grid_transport_state, i)->partner_status)
         << i);
  }
  ...
  grid_msg_packet_body_append_parameter(
      &response, GRID_CLASS_HEARTBEAT_TYPE_offset,
      GRID_CLASS_HEARTBEAT_TYPE_length,
      grid_msg_get_heartbeat_type(&grid_msg_state));
  grid_msg_packet_body_append_parameter(
      &response, GRID_CLASS_HEARTBEAT_HWCFG_offset,
      GRID_CLASS_HEARTBEAT_HWCFG_length, grid_sys_get_hwcfg(&grid_sys_state));
  grid_msg_packet_body_append_parameter(
      &response, GRID_CLASS_HEARTBEAT_VMAJOR_offset,
      GRID_CLASS_HEARTBEAT_VMAJOR_length, GRID_PROTOCOL_VERSION_MAJOR);
  grid_msg_packet_body_append_parameter(
      &response, GRID_CLASS_HEARTBEAT_VMINOR_offset,
      GRID_CLASS_HEARTBEAT_VMINOR_length, GRID_PROTOCOL_VERSION_MINOR);
  grid_msg_packet_body_append_parameter(
      &response, GRID_CLASS_HEARTBEAT_VPATCH_offset,
      GRID_CLASS_HEARTBEAT_VPATCH_length, GRID_PROTOCOL_VERSION_PATCH);

  grid_msg_packet_body_append_parameter(
      &response, GRID_CLASS_HEARTBEAT_PORTSTATE_offset,
      GRID_CLASS_HEARTBEAT_PORTSTATE_length, portstate);

  if (grid_msg_get_heartbeat_type(&grid_msg_state) ==
      1) { // I am usb connected deevice
    ...
    grid_msg_packet_body_append_parameter(
        &response, GRID_CLASS_PAGEACTIVE_PAGENUMBER_offset,
        GRID_CLASS_PAGEACTIVE_PAGENUMBER_length, grid_ui_state.page_activepage);
...
void grid_port_process_outbound_ui(struct grid_port *por) {
  ...
  uint8_t id = grid_msg_string_get_parameter(message, GRID_BRC_ID_offset,
                                             GRID_BRC_ID_length, &error);

  uint8_t dx = grid_msg_string_get_parameter(message, GRID_BRC_DX_offset,
                                             GRID_BRC_DX_length, &error);
  uint8_t dy = grid_msg_string_get_parameter(message, GRID_BRC_DY_offset,
                                             GRID_BRC_DY_length, &error);

  uint8_t sx = grid_msg_string_get_parameter(message, GRID_BRC_SX_offset,
                                             GRID_BRC_SX_length, &error);
  uint8_t sy = grid_msg_string_get_parameter(message, GRID_BRC_SY_offset,
                                             GRID_BRC_SY_length, &error);

  uint8_t rot = grid_msg_string_get_parameter(message, GRID_BRC_ROT_offset,
                                              GRID_BRC_ROT_length, &error);
  uint8_t portrot = grid_msg_string_get_parameter(
      message, GRID_BRC_PORTROT_offset, GRID_BRC_PORTROT_length, &error);
