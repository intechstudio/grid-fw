...
void grid_ui_element_init(struct grid_ui_model* mod, uint8_t element_index, enum grid_ui_element_t element_type){


	struct grid_ui_element* ele = &mod->element[element_index];
	...
	ele->template_a_parameter_list = malloc(GRID_TEMPLATE_A_PARAMETER_LIST_LENGTH*sizeof(uint32_t));
	
	// initialize all of the template parameter values
	for(uint8_t i=0; i<GRID_TEMPLATE_A_PARAMETER_LIST_LENGTH; i++){
		ele->template_a_parameter_list[i] = 0;
	}
	
	ele->template_a_parameter_list[GRID_TEMPLATE_A_PARAMETER_CONTROLLER_NUMBER] = element_index;
...
void grid_ui_event_init(struct grid_ui_event* eve, enum grid_ui_event_t event_type){
	...
	// Allocate and Initialize the EVENT STRING
	...
	// Allocate and Initialize the ACTION STRING
...
// 		for (uint8_t j=0; j<GRID_UI_ACTION_PARAMETER_maxcodelength; j++)
// 		{
// 			eve->action_parameter_list[i].code[j]=0;
// 		}
...
void grid_ui_event_register_action(struct grid_ui_element* ele, enum grid_ui_event_t event_type, uint8_t* action_string, uint32_t action_string_length){
	...
	struct grid_ui_action_parameter parameter_list[GRID_UI_ACTION_PARAMETER_maxcount];
	
	for (uint32_t i=0; i<action_string_length; i++){
		
		// if current character is A and the next character is a number from 0 to 9
		if (action_string[i] == 'A' && (action_string[i+1]-'0') < GRID_TEMPLATE_A_PARAMETER_LIST_LENGTH){
						
// 			parameter_list[parameter_list_length].code[0] =	action_string[i];
// 			parameter_list[parameter_list_length].code[1] =	action_string[i+1];
						
			parameter_list[parameter_list_length].status = GRID_UI_STATUS_INITIALIZED;
			
			parameter_list[parameter_list_length].group = action_string[i];
			
			parameter_list[parameter_list_length].address = (action_string[i+1]-'0');
			parameter_list[parameter_list_length].offset = i;
			parameter_list[parameter_list_length].length = 2;
			parameter_list_length++;
	
		}
		else if (action_string[i] == 'B' && (action_string[i+1]-'0') < GRID_TEMPLATE_B_PARAMETER_LIST_LENGTH){
...
uint8_t grid_ui_event_render_action(struct grid_ui_event* eve, uint8_t* target_string){
	...
	for(uint32_t i=0; i<eve->action_string_length; i++){
		target_string[i] = eve->action_string[i];
	}
	
	return eve->action_string_length;
...
uint8_t grid_ui_event_template_action(struct grid_ui_element* ele, uint8_t event_index){
	...
	for (uint8_t i=0; i<ele->event_list[event_index].action_parameter_count; i++){
		...
		if (ele->event_list[event_index].action_parameter_list[i].group == 'A'){
			...
			uint32_t parameter_value =  ele->template_a_parameter_list[ele->event_list[event_index].action_parameter_list[i].address];
